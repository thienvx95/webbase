...
export class AWSFileUploadSerivce implements IFileUploader {
  ...
  private generateFileKey(file: Express.Multer.File, timestamp: number): string {
    ...
    return `${utcMoment.year()}/${utcMoment.month() + 1}/${utcMoment.date()}/${timestamp}${path.extname(file.originalname)}`;
  } 
  ...
  private async uploadFile(file: Express.Multer.File): Promise<FileUploadDto> {
    ...
    return {
      ...
      path: fileKey,
      type: file.mimetype.split("/") && file.mimetype.split("/").length ? file.mimetype.split("/")[0] : '',
      storage: StorageProvider.S3
    } as FileUploadDto;
  }

  upload = async (files: Express.Multer.File[]): Promise<FileUploadDto[]> => {
    try {
